...
with st.sidebar:
    ...
    st.markdown(
        "<h3 style='color:green'>💰 Créditos: {:,} 🐍</h3>".format(
            st.session_state.cobra_credits
        ),
    ...
    if st.button("Create Video"):
        ...
        if st.session_state.cobra_credits < cost_video:
            ...
            with st.spinner("Generando video..."):
                ...
                try:
                    base_prompt = (
                        f"{video_prompt}, same character, same style, smooth animation, cinematic"
                    )
...
# -------------------
    ...
    st.markdown(
        "<h2 style='color:green; text-align:center;'>💰 Créditos actuales: {:,} 🐍</h2>".format(
            st.session_state.cobra_credits
        ),
        unsafe_allow_html=True
    )
    ...
    elite_credits = round_down_10(185000)
